...
//	mes "Daily Hunting: ^0000FF25 Exp Points^000000";
	mes "Daily Gathering A: ^0000FF50 Exp Points^000000";
	mes "Daily Gathering B: ^0000FF50 Exp Points^000000";
	mes "Daily Check-In: ^0000FF50 Exp Points^000000";
	mes " ";
...
//	switch(select((#hunting < 1)?"Hunting Quests":"",
	switch(select((#collection1 < 1)?"Collection A Quests":"",
				(#collection2 < 1)?"Collection B Quests":"",
				(#checkin < 1)?"Check-in":"",
			  "Exit")) {
	case 1:
//		next;
...
//		}
		next;
		clear;
		doevent("compegather1::OnGather");
		end;
	case 2:
//		doevent("Questboard2::OnCollectionStart");
		next;
		clear;
		doevent("compegather2::OnGather");
		end;
...
//		query_sql("DELETE FROM `acc_reg_num` WHERE `key` = '#hunting'");
		query_sql("DELETE FROM `acc_reg_num` WHERE `key` = '#collection1'");
